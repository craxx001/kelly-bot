...
BOT_TOKEN = os.getenv("8617664721:AAFzgh7TwfJPhAW4JZtlnVAIoiSw6WRx1ZA")
...
def load_data():
    global allowed_groups, known_groups, daily_stats, user_limits
    ...
    try:
        with open(DATA_FILES["groups"], "r", encoding="utf-8") as f:
...
def save_all():
    ...
    with open(DATA_FILES["groups"], "w", encoding="utf-8") as f:
        json.dump(known_groups, f, indent=2)
...
def buy_autolike_keyboard():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔄 Buy Autolike", url="https://t.me/princezz_bot")]
    ])
...
# ============================================================
# GROUP TRACKING / ADMIN BROADCAST
# ============================================================

def remember_group(update: Update):
    ...
    changed = (
        gid not in known_groups
        or known_groups[gid].get("name") != title
    )
...
async def msg_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Send to every group the bot has previously seen/recorded.
    for gid in list(known_groups.keys()):
        try:
...
async def track_bot_chat_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def like_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not can_user_like(user_id):
        ...
        await reply(
            ...
            "💡 Try again tomorrow.",
            reply_markup=buy_autolike_keyboard(),
        )
        return
    ...
    await proc_msg.edit_text(
        result,
        parse_mode="Markdown",
        reply_markup=buy_autolike_keyboard() if status in (1, 2) else None,
    )
...
def main():
    ...
    # Automatically track every group where the bot is added/removed.
